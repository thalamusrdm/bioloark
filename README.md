...
### בלוג Shopify ו־Instagram

- עמוד `/blog` קורא מאמרים מ־Shopify Storefront API באמצעות הרשאת `unauthenticated_read_content`. אם אין מאמרים זמינים, מוצגים ארבעת מדריכי ברירת המחדל שב־`data/articles.ts`.
- כדי להציג את הרילס המובילים, הגדירו `INSTAGRAM_REEL_URLS` כרשימה מופרדת בפסיקים של עד שלוש כתובות Reel מלאות. ללא ההגדרה יוצגו כרטיסים חזותיים המקשרים לפרופיל Instagram, במקום הטמעות שבורות.
