...
export default async function Home() {
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(organization) }} />
    <section className="hero" id="top"><div className="hero-image" aria-hidden="true" /><div className="hero-veil" aria-hidden="true" /><div className="hero-content"><p className="eyebrow">BOTANICAL ART DESIGN FOR INTERIORS</p><h1>יוצרת טרריומים אלגנטיים ועולמות קטנים של טבע שמכניסים חיים ויופי לכל חלל Bioloark</h1><p className="hero-copy">אנחנו משלבים בין עיצוב, איזון ותשומת לב לפרטים כדי ליצור מראה נקי, מרשים ומלא השראה</p><a className="primary-button" href="#collection">למוצרי החנות <span>←</span></a></div><div className="hero-note"><span>01</span><p>עיצוב בוטני בהשראת מסורת הזן היפנית</p></div></section>

    <section className="collection-section" id="collection"><div className="section-heading"><div><p className="eyebrow dark">A CURATED LIVING COLLECTION</p><h2>נבחרים מהסטודיו</h2></div><a href="/category/all-products">לכל המוצרים <span>←</span></a></div><div className="product-grid">{featured.map((product, index) => product && <ProductCard key={product.id} product={product} priority={index < 2} />)}</div></section>

    <section className="terrarium-feature"><img src="/images/terrarium-feature.webp" alt="טרריום מעוצב של Bioloark" /><div className="terrarium-feature-copy"><p className="eyebrow">A WORLD WITHIN</p><h2>טבע חי.<br />בתוך עולם משל עצמו.</h2><p>קומפוזיציות בוטניות חד־פעמיות, שנבנות שכבה אחר שכבה וממשיכות להשתנות יחד עם החלל.</p><a className="outline-button" href="/category/טרריומים-מעוצבים">לחנות טרריומים מעוצבים</a></div></section>

    <section className="services-section"><div className="services-intro"><p className="eyebrow dark">BOTANICAL SYSTEMS & CONSULTING</p><h2>שירותי ייעוץ ותכנון מערכות צמחייה</h2><p>אנו מציעים פתרונות תכנון וליווי אישיים ליצירת מערכות אקולוגיות מתקדמות, המשלבות את העדינות היפנית עם טכנולוגיה מודרנית לחללי יוקרה, משרדים ובתי מלון.</p></div><div className="service-grid">{services.map((service) => <article className="service-card" key={service.number}><div className="service-image"><img src={service.image} alt={service.title} loading="lazy" /><span>{service.number}</span></div><h3>{service.title}</h3><p>{service.description}</p></article>)}</div></section>

    <section className="about-section" id="about"><div className="about-image"><img src="/images/og.jpg" alt="עולם בוטני מיניאטורי בתוך טרריום זכוכית" loading="lazy" /></div><div className="about-copy"><p className="eyebrow dark">קצת עלינו</p><h2>ביולוארק נולדה מתוך אהבה עמוקה לטבע.</h2><p className="about-lead">אנחנו מתמחים ביצירת טרריומים מעוצבים, מערכות צמחייה מתקדמות וקירות ירוקים, המכניסים שלווה, יוקרה וירוק חי אל בתים, משרדים ומלונות.</p><p>כל פרויקט אצלנו מתוכנן בקפדנות – מאפיון החלל, בחירת הצמחים המתאימים ועד תחזוקה קלה לאורך זמן. המטרה שלנו היא ליצור עבורכם פינה ירוקה ייחודית, מותאמת אישית, שנראית כמו יצירת אמנות חיה ונשארת יפה לאורך שנים.</p><a href="/project-showcase">לפרויקטים שלנו <span>←</span></a></div></section>

    <section className="testimonials-section"><div className="section-heading light"><div><p className="eyebrow">LIVING TESTIMONIALS</p><h2>מה אומרים עלינו</h2></div><span className="quote-mark">״</span></div><div className="testimonial-grid">{testimonials.map(([quote, author], index) => <blockquote key={author}><span>0{index + 1}</span><p>{quote}</p><cite>{author}</cite></blockquote>)}</div></section>

    <section className="contact-band"><p className="eyebrow dark">LET&apos;S CREATE SOMETHING LIVING</p><h2>הטבע הבא בחלל שלכם<br />מתחיל בשיחה.</h2><p>זמינים לייעוץ ותכנון מערכות צמחייה מתקדמות לבתי מלון, משרדים וחללי יוקרה.</p><a className="primary-button dark-button" href="mailto:info@bioloark.co.il">יצירת קשר וייעוץ <span>←</span></a><small>ביולוארק — מאפו 13, מרכז תל אביב, ישראל</small></section>
  </main>;
}
