...
export default async function CategoryPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  return <main className="internal-page"><section className="collection-hero"><p className="eyebrow">BIOLOARK COLLECTION</p><h1>{collection.title}</h1><p>{collection.description || 'פריטים שנבחרו בקפידה ליצירת עולמות בוטניים חיים.'}</p></section><section className="collection-body">{handle === 'all-products' && <nav className="collection-category-filter" aria-label="סינון מוצרים לפי קטגוריה"><a href="/category/כלים-להכנה"><span>01</span>הכל לטרריום</a><a href="/category/rare-plants"><span>02</span>צמחים</a><a href="/category/טרריומים-מעוצבים"><span>03</span>יצירות מוכנות</a></nav>}<div className="breadcrumbs"><a href="/">בית</a><span>/</span><span>{collection.title}</span></div><CollectionBrowser products={products} /></section></main>;
}
