    ...
    <div className="drawer-layer" role="dialog" aria-modal="true" aria-label="סל קניות">
      ...
      <aside className="cart-drawer">
        ...
        {!shopifyEnabled && !cart?.lines.length ? <div className="preview-cart"><span>Preview</span><h3>הסל מוכן לבדיקה</h3><p>הוסיפו מוצר כדי לבדוק את חוויית סל הקניות. התשלום המאובטח יופעל בחיבור ל־Shopify.</p></div> : cart?.lines.length ? <>
          ...
          <div className="cart-summary"><div><span>סכום ביניים</span><strong>{shekels(cart.subtotal.amount, cart.subtotal.currencyCode)}</strong></div><p>אפשרויות משלוח ואיסוף עצמי יוצגו בקופה.</p>{shopifyEnabled && cart.checkoutUrl ? <a className="checkout-button" href={cart.checkoutUrl}>לתשלום מאובטח ב־Shopify</a> : <a className="checkout-button" href="mailto:info@bioloark.co.il">להשלמת הזמנה צרו קשר</a>}</div>
